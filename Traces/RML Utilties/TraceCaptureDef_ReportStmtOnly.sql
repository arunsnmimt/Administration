...
Trace definition to support statement level capture only.
Collects statement starting and completed level events, Recompiles, Most error and warning

If you want to capture additional events, such as execution plans, 
you can 'uncomment' back in these events in the script below.

This works for reporting feature only.  Will not work for any of the replay capability.
...
set @maxfilesize = 1024				--	An optimal size for tracing and handling the files
...
exec @rc = sp_trace_create @TraceID output, 0 /* Don't rollover*/, N'InsertFileNameHere', @maxfilesize, NULL 
if (@rc != 0) goto error
